				...
				$("#player").jPlayer({
					ready: function () {
						$(this).jPlayer("setMedia", {
							mp3: "http://stream.r-a-d.io:1130/main.mp3"
						});
						$(".jp-play").attr("disabled", false);
					},
					supplied: "mp3",
					swfPath: window.swfpath,
					cssSelectorAncestor: "#links"
				});
	...
	$.ajax({
			...
			success: function (resp) {
				if (resp.length > 0) {
					...
					$.each(resp, function (i, item) {
						tbody.append(html_row.format(timeConverter(item[0]), item[1]));
					});
				}
				else {
...
function apply_list(which, list) {
	...
	if (list == null) {
		...
		$.each(list, function (i, item) {
			...
			$("#{0} tbody".format(which)).append(html_row.format(time, item[1]));
		});
	}
}
...
function apply_progress() {
	if (update_progress != false) {
		...
		$("#current_pos").html(timeConverter(current_pos, true));
		$("#current_len").html(timeConverter(current_len, true));
	}
	else {
...
$(function () {
	...
	$("#player").jPlayer({
		ready: function () {
			$(this).jPlayer("setMedia", {
				//oga: "http://r-a-dio.com:8000/relay.ogg?_t={0}".format(Math.round(new Date().getTime()/1000.0)),
				mp3: "http://stream.r-a-d.io:1130/main.mp3?_t={0}".format(Math.round(new Date().getTime()/1000.0))
			});
			$(".jp-play").attr("disabled", false);
		},
		supplied: "mp3",
	...
	$("#links a, #links .jp-play, #page-p-staff .media-grid a").twipsy({animate: true, placement: "below", trigger: "hover", fallback: "Default"});
	...
	if (djmode == 14) {
		...
		$('#intro h2').first().before('<a class="fill btn info" onclick="runrave();" style="float:right">ENABLE RAVE MODO</a>');
	}
});
...
function runrave()
{
	...
	$('#intro .btn').first().css('display', 'none');
	$('html').css('height','100%');
	$('body').css('height','100%');
	party = $('#party').first();
	rave();
