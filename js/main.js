...
function fix_url () {
	...
	if (url != false) {
		location = window.location.protocol + "//" + location.hostname + "/" + url;
	}
}
fix_url();
var sync_seconds = 0;
...
function get_page(page, url, finish, postprocess) {
	...
	if (url == false) {
		if (page == "p-staff") {
			...
			finish = function () {
				staff_divs = $("#page-p-staff .djinfo");
				left_over_staff = staff_divs.length % (Math.floor(staff_divs.length / 5) * 5)- (djmode == 6 ? 2 : 0);
							
				if(left_over_staff === 0) { left_over_staff += 5 }

				last_staff_divs  = staff_divs.slice(-left_over_staff);
				other_staff_divs = staff_divs.slice(0, -left_over_staff);

				last_staff_divs.twipsy ({animate: true, placement: "above", trigger: "hover", fallback: "Default"});
				other_staff_divs.twipsy({animate: true, placement: "below", trigger: "hover", fallback: "Default"});
			};
		}
		else if (page == "p-home") {
			url = "/home.php";
			finish = function () {
				...
				$("#player").jPlayer({
					ready: function () {
						//$(this).jPlayer("setMedia", {'mp3':window.location.protocol + '//r-a-d.io/main.mp3'});
						$(".jp-play").attr("disabled", false);
						$(".jp-play").click(function() {
							if(player_source_set == false) {
								$("#player").jPlayer("setMedia", {'mp3':window.location.protocol + '//r-a-d.io/main.mp3'});
				...
				success: function (resp) {
					if ($.isEmptyObject(resp) != true) {
						...
						$.each(resp.result, function (i, item) {
							...
							$("#results tbody").append(html_row.format(item[0], item[1], timeConverter(item[2], TC_MODE_DATE), timeConverter(item[3], TC_MODE_DATE), item[4], btn_disabled, btn_disable));
						});
...
$(function () {
	...
	$("#player").jPlayer({
		ready: function () {
			//$(this).jPlayer("setMedia", {'mp3':window.location.protocol + '://r-a-d.io:1130/main.mp3'});
			$(".jp-play").attr("disabled", false);
			$(".jp-play").click(function() {
				if(player_source_set == false) {
					$("#player").jPlayer("setMedia", {'mp3':window.location.protocol + '//r-a-d.io/main.mp3'});
	...
	if (djmode == 14) {
		...
		$('#streamconnect .browplay-jp').first().after('<a class="fill btn info" id="rave-button" onclick="runrave();" style="margin-top: 3px">Enable Rave Mode</a>');
	}
});
...
function runrave()
{
	if(party != null) {
		...
		$("#logo-image-div").css('background-image', 'url("/res/img/logo_eku.png")');
		party.remove();
		rave1.css('box-shadow', rave1cache);
		rave2.css('box-shadow', rave2cache);
		rave3.css('box-shadow', rave3cache);
        rave4.css('background', 'none');
		
		$('#rave-button').text('Enable Rave Mode')
	...
	$("#logo-image-div").css('background-image', 'url("/res/img/eku_rave_flan.png")');
    ...
    rave1cache = rave1.css('box-shadow');
    rave2cache = rave2.css('box-shadow');
    rave3cache = rave3.css('box-shadow');
	...
	$('#rave-button').text('Disable Rave Mode')
	$('html').css('height','100%');
	$('body').css('height','100%');
	party = $('#party').first();
	rave();
