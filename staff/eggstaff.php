...
$djs = mysql_query("SELECT * FROM `djs` WHERE `visible`='1' and (id=6 or id=7) ORDER BY `priority` ASC;");
$num = mysql_num_rows($djs);
$i = 0;

$site_djs = '<span class="eggmoders">fine gentlemen who<br />made eggmode possible</span><ul class="media-grid">';

while($i < $num) {
	$djname = mysql_result($djs, $i, "djname");
	$djtext = htmlspecialchars(mysql_result($djs, $i, "djtext"));
...
$djs = mysql_query("SELECT * FROM `djs` WHERE `visible`='1' and id!=6 and id!=7 ORDER BY `priority` ASC;");
$num = mysql_num_rows($djs);
$i = 0;
